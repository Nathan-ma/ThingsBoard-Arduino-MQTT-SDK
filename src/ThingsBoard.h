...
// Telemetry record class, allows to store different data using common
// interface.
class Telemetry {
...
#endif
public:
  inline Telemetry() : m_type(TYPE_NONE), m_key(NULL), m_value() {}
  ...
  template <
      ...
      // Standard library is_integral, includes bool, char, signed char,
      // unsigned char, short, unsigned short, int, unsigned int, long, unsigned
      // long, long long, and unsigned long longy
      typename std::enable_if<std::is_integral<T>::value>::type * = nullptr>
#else
      // Workaround for ArduinoJson version after 6.21.0, to still be able to
      // access internal enable_if and is_integral declarations, previously
      // accessible with ARDUINOJSON_NAMESPACE
      typename ArduinoJson::ARDUINOJSON_VERSION_NAMESPACE::detail::enable_if<
          ArduinoJson::ARDUINOJSON_VERSION_NAMESPACE::detail::is_integral<
...
public:
  ...
  inline RPC_Callback() : m_name(), m_cb(NULL) {}
...
private:
  ...
  void process_message(char *topic, uint8_t *payload, unsigned int length) {
    RPC_Response r;
    {
      ...
      for (size_t i = 0; i < sizeof(m_rpcCallbacks) / sizeof(*m_rpcCallbacks);
           ++i) {
        if (m_rpcCallbacks[i].m_cb &&
            !strcmp(m_rpcCallbacks[i].m_name, methodName)) {
  ...
  bool sendDataArray(const Telemetry *data, size_t data_count,
                     bool telemetry = true) {
  ...
  RPC_Callback m_rpcCallbacks[NumRPCCallbacks]; // RPC callbacks array, using
                                                // template param for size

  // PubSub client cannot call a method when message arrives on subscribed
  // topic. Only free-standing function is allowed. To be able to forward event
  // to an instance, rather than to a function, this pointer exists.
  ...
  static void on_message(char *topic, uint8_t *payload, unsigned int length) {
    ...
    ThingsBoardSized::m_subscribedInstance->process_message(topic, payload,
                                                            length);
  }
};
...
template <size_t PayloadSize, size_t MaxFieldsAmt, typename Logger,
          size_t NumRPCCallbacks>
ThingsBoardSized<PayloadSize, MaxFieldsAmt, Logger, NumRPCCallbacks>
    *ThingsBoardSized<PayloadSize, MaxFieldsAmt, Logger,
                      NumRPCCallbacks>::m_subscribedInstance;
...
public:
  ...
  inline ThingsBoardHttpSized(Client &client, const char *access_token,
      ...
      : m_actual_client(client), m_host(host), m_token(access_token),
        m_port(port) {}
